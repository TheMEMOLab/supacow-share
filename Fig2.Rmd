---
title: "| Fig2 for Kobel *et al.*: Protozoal populations drive system-wide variation in the rumen microbiome\n"
author: "VTEA"
...
dig_pc_loadings_top_trim$tax_label <- factor(dig_pc_loadings_top_trim$tax_label,
                                             levels = c(
                                               setdiff(
                                                 sort(unique(dig_pc_loadings_top_trim$tax_label)), "other"),
                                             "other"))
...
```{r fig2, fig.width=7, fig.height=10}
plot_grid(
  plot_grid(
      asv_pcoa + ggtitle("a"),
      mag_pcoa + ggtitle("b"),
  ...
  pc_boxplots_host,
  pc_loadings_tax,
  ncol = 1, rel_heights = c(1, 0.75, 0.9, 0.85))
```
...
# for revisions: instead of simple counts of species seen,
# summarize actual values of loadings per taxon
...
# order by sum of contribution
dig_pc_loadings_per_tax <- dig_pc_loadings_per_tax[order(dig_pc_loadings_per_tax$load_contrib),]
dig_pc_loadings_per_tax$tax <- factor(dig_pc_loadings_per_tax$tax,
                                      levels = rev(unique(dig_pc_loadings_per_tax$tax)))

# total loadings (untrimmed) for the whole PCA
ggplot(dig_pc_loadings_per_tax,
         aes(y = tax, x = load_contrib, fill = side)) +
    geom_col(stat = "identity",
             position = position_dodge2(width = 1, preserve = "single")) +
    ylab(NULL) + xlab(NULL) +
    theme_classic(base_size = 8) +
    scale_fill_manual(values = RCT_cols) +
    facet_grid(db~source, scale = "free", space = "free") +
    theme(plot.title = element_text(face = "bold", size = 12),
            legend.position = "none")
```

```{r pc_load_domain, fig.width=10, fig.height=2.5}
# Instead of per-taxon, show summaries per database / domain + side:
...
contribs_p_domain <- ggplot(dig_pc_loadings_per_domain,
    ...
    geom_col(stat = "identity",
             position = position_dodge2(width = 1, preserve = "single")) +
  ylab(NULL) +
  xlab("% contribution to PC1 loadings") +
    theme_bw(base_size = 8) +
    scale_fill_manual(values = rev(RCT_cols)) +
    facet_grid(.~source, scale = "free", space = "free") +
    theme(plot.title = element_text(face = "bold", size = 12),
          panel.grid.minor = element_blank(),
          strip.background = element_rect(fill = "white"))
```

```{r pc_load_trim, fig.width=10, fig.height=8}
# trimming: keep only those taxa that have a total contribution
# of more than 1% (0.01) in at least one side/source
top_contrib_tax <- unique(
  subset(dig_pc_loadings_per_tax, load_contrib > 0.01)$tax)
dig_pc_loadings_per_tax_trim <- subset(dig_pc_loadings_per_tax,
                                       tax %in% top_contrib_tax)

# Order levels by total contribution across everything
top_contrib_order <- aggregate(load_contrib ~ tax,
                               dig_pc_loadings_per_tax_trim,
                               sum)
top_contrib_order <- top_contrib_order[order(top_contrib_order$load_contrib),]

dig_pc_loadings_per_tax_trim$tax <- factor(dig_pc_loadings_per_tax_trim$tax,
                                           levels = top_contrib_order$tax)
...
```{r}
# combo
plot_grid(contribs_p_domain,
          contribs_p_genus,
          ncol = 1, rel_heights = c(0.2, 0.8), align = "hv", axis = "l")
```

```{r loadings_summary}
# trim (top per side+source)
dig_pc_loadings_per_tax_trim <- rbind(
  head(subset(dig_pc_loadings_per_tax,
         source == "Digesta_metatranscriptomics"), 10),
  tail(subset(dig_pc_loadings_per_tax,
         source == "Digesta_metatranscriptomics"), 10),
  head(subset(dig_pc_loadings_per_tax,
         source == "Digesta_metaproteomics"), 10),
  tail(subset(dig_pc_loadings_per_tax,
         source == "Digesta_metaproteomics"), 10))
  
dig_pc_loadings_per_tax_trim$tax <- factor(dig_pc_loadings_per_tax_trim$tax,
                                      levels = sort(unique(dig_pc_loadings_per_tax_trim$tax)))

ggplot(dig_pc_loadings_per_tax_trim,
         aes(x = tax, y = source, fill = loading)) +
    geom_tile(color = "gray70") +
    geom_text(color = "gray25", size = 2.5, aes(label = round(loading, digits = 2))) +
    ylab(NULL) + xlab(NULL) +
    theme_classic(base_size = 8) +
    scale_fill_gradient2(low = "#49ad8b",
                         mid = "white",
                         high = "gray10",
                         midpoint = 0) +
    facet_grid(side~db, scale = "free_x", space = "free_x") +
...
ggplot(dig_pc_loadings_per_tax_trim, aes(y = tax, fill = side, x = loading)) +
    geom_bar(stat = "identity") + theme_bw() +
    facet_grid(side~source, scale = "free", space = "free")


# totals for protozoa vs prokaryotes:
loadings_ag_db <- aggregate(loading ~ db + side + source, dig_pc_loadings_per_tax, "sum")

ggplot(loadings_ag_db, aes(y = db, fill = side, x = loading)) +
    geom_bar(stat = "identity") + theme_bw() +
    facet_grid(side~source, scale = "free", space = "free")
```
...
# for revisions: instead of simple counts of species seen,
# summarize actual values of loadings per taxon
# IN the top 100 loadings (as seen in Fig 2g) per side only!

dig_pc_loadings_top100 <- rbind(
  data.frame(
    dig_metat_pc_loadings[
      order(dig_metat_pc_loadings$loading)[1:100], c("tax", "db", "loading")],
    side = "negative",
    source = "Digesta_metatranscriptomics"),
  data.frame(
    dig_metat_pc_loadings[
      order(dig_metat_pc_loadings$loading, decreasing = TRUE)[1:100], c("tax", "db", "loading")],
    side = "positive",
    source = "Digesta_metatranscriptomics"),
  data.frame(
    dig_metap_pc_loadings[
      order(dig_metap_pc_loadings$loading)[1:100], c("tax", "db", "loading")],
    side = "negative",
    source = "Digesta_metaproteomics"),
  data.frame(
    dig_metap_pc_loadings[
      order(dig_metap_pc_loadings$loading, decreasing = TRUE)[1:100], c("tax", "db", "loading")],
    side = "positive",
    source = "Digesta_metaproteomics"))

# Fix/simplify discrepant naming
dig_pc_loadings_top100[dig_pc_loadings_top100$tax == "Ruminococcus_E", "tax"] <- "Ruminococcus"

# sums of loadings per taxa
dig_pc_loadings_top100_a <- aggregate(loading ~ db + tax + source + side,
                                      data = dig_pc_loadings_top100, FUN = "sum")

# order by sum of loadings
dig_pc_loadings_top100_a <- dig_pc_loadings_top100_a[order(dig_pc_loadings_top100_a$loading),]


ggplot(dig_pc_loadings_top100_a,
         aes(x = tax, y = source, fill = loading)) +
    geom_tile(color = "gray70") +
    geom_text(color = "gray25", size = 2.5, aes(label = round(loading, digits = 2))) +
    ylab(NULL) + xlab(NULL) +
...
# with "other" category matching the taxa in 2g
dig_pc_loadings_top100_a$tax_fix <- ifelse(dig_pc_loadings_top100_a$tax %in%
                                             dig_pc_loadings_top_trim$tax,
                                           dig_pc_loadings_top100_a$tax,
                                           "other")

# re-aggregate
dig_pc_loadings_top100_a2 <- aggregate(loading ~ db + tax_fix + source + side,
                                      data = dig_pc_loadings_top100_a, FUN = "sum")

ggplot(dig_pc_loadings_top100_a2,
         aes(x = tax_fix, y = source, fill = loading)) +
    geom_tile(color = "gray70") +
    geom_text(color = "gray25", size = 2.5, aes(label = round(loading, digits = 2))) +
    ylab(NULL) + xlab(NULL) +
    theme_classic(base_size = 8) +
