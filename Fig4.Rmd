...
levels(sel_func_summary$func_facet)[
  grep(",", levels(sel_func_summary$func_facet))] <- "CAZymes:\nmultiple\nsubstrates"
...
levels(
  sel_func_summary$func_facet)[grep("reference",
                                    levels(sel_func_summary$func_facet))] <- "CAZymes:\nmultiple\nsubstrates"
...
# Adjust longer genus names for plotting
levels(sel_func_summary_sags_trim$Genus) <- sub("Eremoplastron", "Eremo-\nplastron",
                                                sub("Ostracodinium", "Ostraco-\ndinium",
...
```{r combo_fig4, fig.width=4, fig.height=10}
...
p_metab <- ggplot(feature_comp_summary_trim, aes(y = label, fill = RCT)) +
  ...
  geom_text(aes(label = star), x = 1.65) +
...
```{r fig4_export, fig.width=7, fig.height=11}
plot_grid(
  ...
  ncol = 1, rel_heights = c(0.575, 0.425))
```
