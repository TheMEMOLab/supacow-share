---
title: "| Fig 2, Supplementary Fig 4 & Supplementary Data 3 for Kobel *et al.*: Protozoal populations drive system-wide variation in the rumen microbiome\n"
author: "VTEA"
...
# Calculate adonis2 p-value
asv_pcoa_p <- adonis2(pcoa_asv_res[["dist"]]~RCT,
                      data = as(sample_data(asv_phy_t6), "data.frame"),
                      perm = 9999)[["Pr(>F)"]][1]
# simplify number format
asv_pcoa_p <- format(asv_pcoa_p, scientific = FALSE)
...
asv_pcoa <- ggplot(data = pcoa_asv_df,
  ...
  labs(x = paste("Axis1:", round(pcoa_asv_res$pcoa$values$Relative_eig[1]*100), "% variance"),
       ...
       caption = paste0("p (PERMANOVA, RCT) = ", asv_pcoa_p)) +
...
# Calculate adonis2 p-value
mag_pcoa_p <- adonis2(pcoa_mag_res[["dist"]]~RCT,
                      data = as(sample_data(mag_phy_t6), "data.frame"),
                      perm = 9999)[["Pr(>F)"]][1]
# simplify number format
mag_pcoa_p <- format(mag_pcoa_p, scientific = FALSE)
...
mag_pcoa <- ggplot(data = pcoa_mag_df,
  ...
  labs(x = paste("Axis1:", round(pcoa_mag_res$pcoa$values$Relative_eig[1]*100), "% variance"),
       ...
       caption = paste("p (PERMANOVA, RCT) = ", mag_pcoa_p)) +
  theme(panel.grid = element_blank(),
        legend.box.background = element_rect(color = "black", linewidth = 0.75),
        panel.border = element_rect(linewidth = 0.75),
        axis.text = element_text(color = "black"),
...
pcs_vs_RCT_full <- lapply(1:length(all_pcas_list), function(x){
  ...
  do.call("rbind", lapply(grep("PC", colnames(df), value = TRUE), function(y){
    tres <- t.test(as.formula(paste(y, "~ RCT")), df)
    effsize <- cohens_d(df, as.formula(paste(y, "~ RCT")))
    data.frame(PC = y,
               t_statistic = tres$statistic,
               df = tres$parameter, 
               p = tres$p.value,
               conf_int_95 = paste(round(tres$conf.int, 2), collapse = ", "),
               effect_size = effsize$effsize)
  }))
})
names(pcs_vs_RCT_full) <- names(all_pcas_list)

# Simplified results: only p-values
pcs_vs_RCT <- lapply(pcs_vs_RCT_full, function(x)
  setNames(x$p, x$PC))
...
split_pc_vals$sampletype_PC <- paste(split_pc_vals$sample_type,
                                     split_pc_vals$PC,
                                     paste0("(n = ", split_pc_vals$n_samples, ")"),
                                     sep = "\n")
...
split_pc_vals$p_label <- ifelse(split_pc_vals$pval < 0.0001,
                                "p < 0.0001",
                                paste0("p = ", format(
                                  round(split_pc_vals$pval, digits = 4),
                                  scientific = FALSE)))
...
split_pc_pvals$ypos <- c(rep(20, 2),
                         rep(50, 2),
                         rep(100, 2),
                         rep(21, 4))
split_pc_pvals$ypos_segment <- split_pc_pvals$ypos*0.85
...
pc_boxplots_host <- ggplot(subset(split_pc_vals, plot_grouping == "Host omics"),
  ...
  scale_fill_manual(values = RCT_cols, name = "**Panels  \nc-f:**  \n  \nRCT") +
  geom_dotplot(binaxis = "y", stackdir = "center", dotsize = 1.25,
  ...
  annotate(geom = "text",
           ...
           label = subset(split_pc_pvals, plot_grouping == "Host omics")$p_label,
           size = 2.25) +
  annotate(geom = "segment",
           x = seq(0.85, 3.85, 1),
...
# Export PCA results for Supplementary Data 3

```{r pca_export}
# Overall PCA / omics RCT comparisons: combine to one table
pcs_vs_RCT_full <- do.call("rbind", lapply(names(pcs_vs_RCT_full), function(x)
  data.frame(data = x, pcs_vs_RCT_full[[x]])))
rownames(pcs_vs_RCT_full) <- NULL

# Two PCAs analyzed in more detail:
# list with concatenated sample scores and loadings from selected PCAs
...
# Put all together:
pca_exports_list <- c(
  PCA_vs_RCT = list(pcs_vs_RCT_full),
  pca_list_concat,
  pca_tax_details,
...
```{r pca_supplement_fig, fig.width=7, fig.height=9}
