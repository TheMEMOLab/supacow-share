...
## Summaries per genus (prokaryotes) / species (protozoa)
...
# Set up italics for taxon labels
dig_pc_loadings_per_tax_trim$tax_label <- sapply(
  as.character(dig_pc_loadings_per_tax_trim$tax), function(x)
    ifelse(
      (x %in% grep("[0-9]", levels(dig_pc_loadings_per_tax_trim$tax), value = TRUE) | x == "other"),
         x,
         paste0("*", x, "*")))
# Manual fix for one special case ("Isotricha sp YL-2021a")
dig_pc_loadings_per_tax_trim[dig_pc_loadings_per_tax_trim$tax_label == "Isotricha sp YL-2021a",
                         "tax_label"] <- "*Isotricha* sp. YL-2021a"
